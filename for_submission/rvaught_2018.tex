...
\tablehead{\colhead{Object} & \colhead{R.A.} & \colhead{Dec} & \colhead{$z$} & \multicolumn2c{$\log$(SFR)} & \multicolumn2c{$\log{M}$} & \colhead{EW$_{\rm{obs}}$} & \multicolumn2c{ $\tau_V$} \\ 
\colhead {} & \colhead {} & \colhead {} & \colhead{} & \multicolumn2c{($\log M_{\odot}$ yr$^{-1}$)} & \multicolumn2c{($\log{M_{*}/M_{\odot}}$)} & \colhead{(\AA)} & \multicolumn2c{}}
\decimals
\startdata
J033225.26-274524.0 (J.26)     & 03:32:25.26 & -27:45:23.9  &   0.6660  & $9.1_{-3.7}^{+1.3}$     & $9.86_{-0.04}^{+0.05}$   & \ $7.539\pm 0.354$   &  $1.227_{-0.20}^{+1.54}$ \\ 
J033231.36-274725.0 (J.36)     & 03:32:31.35 & -27:47:24.9  &   0.6669  & $10.5_{-1.6}^{+1.7}$   & $10.02_{-0.03}^{+0.03}$ & \ $5.835 \pm 0.493$   & $1.377_{-0.23}^{+0.60}$ \\
...
\tablecaption{Filter properties  and exposure times of the VLT/FORS2 observations. The widths of the transmission curves ($\Delta\lambda$) are calculated by convolving the transmission curves over the total wavelength range of each filter. These values differ slightly from those reported by the European Southern Observatory. \label{tab:filters}}
...
\colhead {} & \colhead{(\AA)} & \colhead{(\AA)} & \colhead{} & \colhead{(S)} & \colhead{}}
\startdata
...
\tablenotetext{b}{FWHM of the filter.}
...
However, since our filter transmission curves are sampled at wavelengths similar to sampling of the spectrum of GD50 from the the CALSPEC library \citep{Bohlin2017}, we interpolate both spectra and compute the integral numerically. 
...
We construct smoothed $\chi$ images, $\chi_{\text{smooth}}$, for each level of simulated emission, following the techniques in \cite{Hennawi2013} and \cite{Battaia_2015}. These $\chi_{\text{smooth}}$ images can aid in identifying the presence and detectability of extended \ion{Mg}{2} emission.

First, the model continuum-subtracted image is smoothed by the operation:
\begin{equation}
...
\end{equation}
where the CONVOLVE$^2$ operation indicates convolution of the image with the square of the Gaussian kernel. The smoothed $\chi$ image is then the product of the data image divided by the sigma image.
\begin{equation}
...
For our sample of galaxies, we are sensitive to extended emission in annuli at projected distances of 18, 18, 24 and 14 kpc from the centers of J.26, J.36, J.03 and J.64, respectively.
