...
However, since our filter transmission curves are sampled at wavelengths similar to sampling of the spectrum of GD50 from the the CALSPEC library \citep{Bohlin2017}, we interpolate both spectra and compute the integral without the constant $F(\lambda)$ assumption . 
...
To aid in identifying the presence and detectability of extended \ion{Mg}{2} emission, we construct a $\chi_{\text{smooth}}$, for each level of simulated emission, following the techniques in \cite{Hennawi2013} and \cite{Battaia_2015}.

To construct this smoothed image, we first performed the following operation on a continuum-subtracted image:
\begin{equation}
